...
{
    public partial class p_AddOtherDeductionsEntry : Form
    {
        ...
        Boolean isnew = false;
        String code = "";
        p_OtherDeductionEntry _frm_earn = null;
        public p_AddOtherDeductionsEntry()
        {
            InitializeComponent();
        ...
        {
            DataTable dt = null;
            dt = db.QueryBySQLCode("SELECT * FROM rssys.hr_deduction_entry WHERE   dedcode='" + pk + "'");
            if (dt.Rows.Count > 0)
            {
        ...
        private void p_AddOtherDeductionsEntry_Load(object sender, EventArgs e)
        {
        ...
        private void button3_Click(object sender, EventArgs e)
        {
            ...
            String table = "hr_deduction_entry";
            ...
            {
                MessageBox.Show("Please Select Deductions.");
            ...
            else
            {
                ...
                if (isnew)
                {
                    entcode = db.get_pk("dedcode");
                    col = "dedcode,payroll_period,emp_no,emp_name,deduction_code,amount";
                    val = "'" + entcode + "', '" + payroll_period + "', '" + emp_no + "', '" + emp_name + "', '" + earning_code + "', '" + amount + "'";

                    if (db.InsertOnTable(table, col, val))
                    {
                        db.set_pkm99("dedcode", db.get_nextincrementlimitchar(entcode, 8));
                        success = true;
                        //add_items(loan_code);
                    }
                    else
                    {
                        success = false;
                        db.DeleteOnTable(table, "dedcode='" + entcode + "'");
                ...
                else
                {
                    ...
                    col = "dedcode='" + entcode + "', payroll_period='" + payroll_period + "', emp_no='" + emp_no + "', emp_name='" + emp_name + "', deduction_code='" + earning_code + "', amount='" + amount + "'";

                    if (db.UpdateOnTable(table, col, "dedcode='" + entcode + "'"))
                    {
