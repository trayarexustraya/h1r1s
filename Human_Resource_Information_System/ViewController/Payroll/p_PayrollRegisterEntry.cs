...
namespace Human_Resource_Information_System
{
    ...
    public partial class p_PayrollRegisterEntry : Form
    {
        p_PayrollRegister _p_frm_reg = null;
        ...
        public p_PayrollRegisterEntry(p_PayrollRegister frm , String pk , Boolean _isnew)
        {
            InitializeComponent();
            gc.load_payroll_period(cbo_period_code);
            gc.load_dept(cbo_department_frm);
        ...
        {
            DataTable dt = null;
            dt = db.QueryBySQLCode("SELECT * FROM rssys.hr_payroll_register WHERE payrollreg_code='"+pk+"'");
            if(dt.Rows.Count > 0)
            {
                for (int i = 0; i< dt.Rows.Count;i++ )
                {
                    txt_code.Text = dt.Rows[i]["payrollreg_code"].ToString();
        ...
        private void button3_Click(object sender, EventArgs e)
        {
            ...
            String table = "hr_payroll_register";
            ...
            else if(cbo_report_type.SelectedIndex == -1)
            {
                ...
                report_type = cbo_report_type.Text;
                      ...
                      if(isnew)
                      {
                          payrollreg_code = db.get_pk("payrollreg_code");
                          col = "payrollreg_code,dept_frm,dept_until,employee,report_type,payroll_period_code,payroll_period_desc";
                          val = "'" + payrollreg_code + "', '" + dept_frm + "', '" + dept_until + "', '" + employee + "', '" + report_type + "', '" + payroll_period_code + "', '" + payroll_period_desc  + "'";

                          if (db.InsertOnTable(table, col, val))
                          {
                              db.set_pkm99("payrollreg_code", db.get_nextincrementlimitchar(payrollreg_code, 8));
                              success = true;
                              //add_items(loan_code);
                          }
                          else
                          {
                              success = false;
                              db.DeleteOnTable(table, "payrollreg_code='" + payrollreg_code + "'");
                          ...
                          col = "payrollreg_code='" + payrollreg_code + "', dept_frm='" + dept_frm + "', dept_until='" + dept_until + "', employee='" + employee + "', report_type='" + report_type + "', payroll_period_code='" + payroll_period_code + "', payroll_period_desc='" + payroll_period_desc + "'";

                          if (db.UpdateOnTable(table, col, "payrollreg_code='" + payrollreg_code + "'"))
                          {
